<template>
  <div class="cloudflare-ddns-container">
    <div class="main-content">
      <!-- Cloudflare 配置卡片 -->
      <div class="config-card">
        <div class="card-header">
          <h3 class="card-title">Cloudflare 配置</h3>
        ...
        <div class="card-body">
          <el-descriptions :column="1" border>
            <el-descriptions-item label="ZoneID" width="250px">
              {{ keyForm.keyID || '未配置' }}
            </el-descriptions-item>
            <el-descriptions-item label="Token" width="350px">
      ...
      <div class="ddns-card">
        ...
        <div class="card-body">
          <el-table :data="dnsList" border style="width: 100%">
            ...
            <el-table-column prop="RR" label="前缀" width="80" align="center" />
    ...
    >
      <el-form 
        ...
        label-width="100px"
      >
        <el-form-item label="ZoneID" prop="keyID">
          <el-input v-model="keyForm.keyID" placeholder="请输入ZoneID" />
        </el-form-item>
        
        <el-form-item label="Token" prop="keySecret">
          <el-input 
    ...
    >
      ...
      >
        ...
        <el-form-item label="IP地址" prop="value" v-if="currentRecord.flagStr === 'define'">
          <el-input v-model="currentRecord.value" />
        </el-form-item>
...
const dnsDialogMode = ref('add') // 'add' or 'edit'
...
const checkLogin = (data) => {
  if (data?.errCode === 1000) {
    ...
    router.push('/user-login?callback=cloudflare-ddns')
    return false
  }
  return true
}
...
const saveCloudflareKey = async () => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/update-cloudflareKey`, {
      method: 'POST',
      headers: {
    ...
    ElMessage.success('Cloudflare配置保存成功')
...
const saveDnsRecord = async () => {
  try {
    ...
    const endpoint = (dnsDialogMode.value === 'edit') ? '/update-cloudflareDns' : '/add-cloudflareDns'
    ...
    ElMessage.success(`DNS记录${dnsDialogMode.value === 'edit' ? '更新' : '新增'}成功`)
...
// 删除DNS记录
const deleteCloudflareDns = async (row) => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/delete-cloudflareDns`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
...
// 切换自动刷新
const toggleAutoRefresh = async (enable) => {
  try {
    const response = await fetch(`${API_BASE_URL}/cloudflareDns?refresh=${enable ? 'on' : 'off'}`, {
      credentials: 'include'
    })
    ...
    ElMessage.success(enable ? '已开启自动刷新' : '已关闭自动刷新')
    if (enable) {
      setupAutoRefresh()
    } else {
      clearAutoRefresh()
...
const setupAutoRefresh = () => {
  if (autoRefresh.value) {
    refreshInterval.value = setInterval(fetchData, 2000)
  }
}
...
const fetchData = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/cloudflareDns`, {
      credentials: 'include'
    })
    ...
    // 更新DDNS列表
    if (data.data?.cloudflareMap) {
      dnsList.value = data.data.cloudflareMap || []
    }
    
    // 更新开关状态
    dnsSwitch.value = data.dnsSwitch
    autoRefresh.value = data.autoRefresh
