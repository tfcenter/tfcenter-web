<template>
  <div class="tencentcloud-ddns-container">
    <div class="main-content">
      <!-- Tencent Cloud 密钥配置卡片 -->
      <div class="config-card">
        <div class="card-header">
          <h3 class="card-title">Tencent Cloud 配置</h3>
        ...
        <div class="card-body">
          <el-descriptions :column="1" border>
            <el-descriptions-item label="SecretId">
              {{ keyForm.secretId || '未配置' }}
            </el-descriptions-item>
            <el-descriptions-item label="SecretKey">
              {{ keyForm.secretKey ? '••••••••' : '未配置' }}
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
              ...
              <template #default="{ row }">
                <div class="action-buttons">
                  ...
                  <el-button
                    type="danger"
                    size="small"
                    @click="deleteTencentcloudDns(row)"
                  >
                    删除
                  </el-button>
                </div>
    ...
    <!-- Tencent Cloud 密钥编辑对话框 -->
    <el-dialog
      v-model="keyDialogVisible"
      title="编辑Tencent Cloud配置"
      width="600px"
    ...
    >
      ...
      >
        <el-form-item label="SecretId" prop="secretId">
          <el-input v-model="keyForm.secretId" placeholder="请输入SecretId" />
        </el-form-item>
...
const checkLogin = (data) => {
  if (data?.errCode === 1000) {
    ...
    router.push('/user-login?callback=tencentcloud-ddns')
    return false
  }
  return true
}
...
// 保存Tencent Cloud密钥
const saveTencentcloudKey = async () => {
  try {
    await keyFormRef.value.validate()
    keyLoading.value = true
    
    const params = new URLSearchParams()
    params.append('secretId', keyForm.value.secretId)
    params.append('secretKey', keyForm.value.secretKey)
    
    const response = await fetch(`${API_BASE_URL}/update-tencentcloudKey`, {
      method: 'POST',
      headers: {
    ...
    ElMessage.success('Tencent Cloud配置保存成功')
    keyDialogVisible.value = false
    fetchData()
  } catch (error) {
    console.error('保存Tencent Cloud配置失败:', error)
...
const saveDnsRecord = async () => {
  try {
    ...
    const endpoint = (dnsDialogMode.value === 'edit') ? '/update-tencentcloudDns' : '/add-tencentcloudDns'
...
// 删除DNS记录
const deleteTencentcloudDns = async (row) => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/delete-tencentcloudDns`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
...
const fetchData = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/tencentcloudDns`, {
      credentials: 'include'
    })
    ...
    if (data.data?.secretId) {
      keyForm.value.secretId = data.data.secretId
      keyForm.value.secretKey = data.data.secretKey
    }
    
    // 更新DDNS列表
    if (data.data?.tencentcloudMap) {
      dnsList.value = data.data.tencentcloudMap || []
    }
