    ...
    <div class="container">
      ...
      <div class="setting-card">
        <div class="card-header">
          <h2><i class="fas fa-sign-in-alt"></i> 登录后跳转页面</h2>
        </div>
        <div class="card-content">
          <el-table :data="[loginPageSettings]" border style="width: 100%">
            <el-table-column prop="loginPage" label="登录后跳转页面" width="300">
              <template #default="{ row }">
                <el-select v-model="row.loginPage" size="small" style="width: 100%">
                  <el-option label="首页" value="sys-home" />
                  <el-option label="中转服务器" value="transfer-server" />
                  <el-option label="端口映射" value="server-mapping" />
                  <el-option label="P2P客户端" value="p2p-client" />
                  <el-option label="正向代理" value="proxy-mgmt" />
                  <el-option label="反向代理" value="reverse-proxy" />
                  <el-option label="用户配置" value="user-mgmt" />
                  <el-option label="文件配置" value="file-setting" />
                  <el-option label="文件列表" value="file-list" />
                  <el-option label="收藏夹" value="collect-list" />
                  <el-option label="分享夹" value="shareLink-list" />
                  <el-option label="文件同步" value="file-sync-mgmt" />

                  <el-option label="阿里云DDNS设置" value="aliyun-ddns" />
                  <el-option label="pubyun DDNS设置" value="pubyun-ddns" />
                  <el-option label="nameCom DDNS设置" value="nameyun-ddns" />
                  <el-option label="tencentcloud DDNS设置" value="tencentcloud-ddns" />
                  <el-option label="cloudflare DDNS设置" value="cloudflare-ddns" />

                  <el-option label="挂载管理" value="mount-mgmt" />

                  <el-option label="系统设置" value="sys-setting" />

                  <el-option label="关于" value="sys-about" />
                </el-select>
              </template>
            </el-table-column>
      ...
      <!-- Web设置卡片 -->
      <div class="setting-card"  v-if="loadCompo?.includes('filemgmt')">
        <div class="card-header">
          <h2><i class="fas fa-cog"></i> Web设置(需重启软件生效)</h2>
        </div>
        <div class="card-content">
          <el-table :data="[webSettings]" border style="width: 100%">
            <el-table-column prop="httpPort" label="HTTP端口号" width="120">
              <template #default="{ row }">
                <el-input v-model="row.httpPort" size="small" />
              </template>
            </el-table-column>
            <el-table-column prop="httpStatus" label="HTTP状态" width="120">
              <template #default="{ row }">
                <el-select v-model="row.httpStatus" size="small">
                  <el-option label="开启" value="on" />
                  <el-option label="关闭" value="off" />
                </el-select>
              </template>
            </el-table-column>
            <el-table-column prop="httpsPort" label="HTTPS端口号" width="120">
              <template #default="{ row }">
                <el-input v-model="row.httpsPort" size="small" />
              </template>
            </el-table-column>
            <el-table-column prop="httpsStatus" label="HTTPS状态" width="120">
              <template #default="{ row }">
                <el-select v-model="row.httpsStatus" size="small">
                  <el-option label="开启" value="on" />
                  <el-option label="关闭" value="off" />
                </el-select>
              </template>
      ...
      <div class="setting-card">
        <div class="card-header">
          <h2><i class="fas fa-user-shield"></i> 免密登录设置(127.0.0.1)</h2>
        </div>
        <div class="card-content">
          <el-table :data="[authSettings]" border style="width: 100%">
            <el-table-column prop="status" label="当前状态" width="120">
              <template #default="{ row }">
                <el-input v-model="row.status" :readonly="true" size="small" />
              </template>
            </el-table-column>
            <el-table-column label="操作" width="120">
              <template #default="{ row }">
                <el-button 
                  type="primary" 
                  size="small" 
                  @click="toggleAuthLogin"
                  :loading="authLoading"
                >
                  {{ row.status === '已开启' ? '关闭' : '开启' }}
                </el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <div class="setting-card">
        <div class="card-header">
          <h2><i class="fas fa-clipboard-list"></i> 日志设置</h2>
        </div>
        <div class="card-content">
          <el-table :data="[logSettings]" border style="width: 100%">
            <el-table-column prop="level" label="系统日志级别" width="250">
              <template #default="{ row }">
            ...
            <el-table-column prop="day" label="日志天数(0-30)" width="150">
              <template #default="{ row }">
                <el-input-number 
                  v-model="row.day" 
                  size="small"
      ...
      <div class="setting-card" v-if="loadCompo?.includes('filemgmt')">
        <div class="card-header">
          <h2><i class="fas fa-hdd"></i> 缓存设置</h2>
        </div>
        <div class="card-content">
          <el-table :data="[cacheSettings]" border style="width: 100%">
            <el-table-column prop="path" label="缓存地址" width="300">
              <template #default="{ row }">
                <el-input v-model="row.path" size="small" />
              </template>
            </el-table-column>
            <el-table-column prop="time" label="缓存时间(秒)" width="150">
              <template #default="{ row }">
                <el-input v-model="row.time" size="small" />
              </template>
            </el-table-column>
            <el-table-column prop="recycle" label="删除文件放回收站" width="180">
              <template #default="{ row }">
                <el-select v-model="row.recycle" size="small">
                  <el-option label="开启" value="on" />
                  <el-option label="关闭" value="off" />
                </el-select>
              </template>
      ...
      <div class="setting-card">
        <div class="card-header">
          <h2><i class="fas fa-lock"></i>安全入口路径</h2>
        </div>
        <div class="card-content">
          <el-table :data="[securitySettings]" border style="width: 100%">
      ...
      <div class="setting-card">
        <div class="card-header">
          <h2><i class="fas fa-file-export"></i> 备份配置</h2>
        </div>
        <div class="card-content">
          <div class="backup-actions">
            <el-button 
              type="primary" 
              @click="backupAllConfig"
      ...
      <div class="setting-card">
        <div class="card-header">
          <h2><i class="fas fa-file-import"></i> 恢复cfg配置</h2>
        </div>
        <div class="card-content">
          <el-upload
            class="upload-demo"
          ...
          >
            ...
            <span class="file-name">{{ uploadFile ? uploadFile.name : '未选择任何文件' }}</span>
            <el-button 
              type="primary" 
              @click="submitUpload" 
              :disabled="!uploadFile"
      ...
      <div class="setting-card">
        <div class="card-header">
          <h2><i class="fas fa-ellipsis-h"></i> 其他操作</h2>
        </div>
        <div class="card-content">
          <el-button 
            type="primary" 
